...
public class LoginController extends AbstractController {
    ...
    public void prompt() {
        ...
        try {
            ...
            Service service = org.choongang.member.services.MemberServiceLocator.getInstance().find(Menu.LOGIN);
            service.process(form);

            router.change(Menu.MAIN); // 로그인 성공시 -> 메인 페이지 이동
        } catch (RuntimeException e) {

            System.err.println(e.getMessage());
            router.change(Menu.LOGIN); // 로그인 실패시 -> 로그인 페이지 이동
        }
    }
}